...
struct big_integer{
    ...
    big_integer get_reverse(){        
        ...
        for(int i(data.size()-1); i >= 0; i--){
            if(data[i] < 100'000'000'000'000'000 && i != data.size()-1)
                for(int j{0}; j < 17-(int)log10(data[i]+0.9); j++) 
                    s += '0';
            s += to_string(data[i]);
        }
    ...
    big_integer operator+(const big_integer &x) const{
        ...
        while(i < x.data.size() && i < data.size()){
            ...
            ans.data.push_back(res % 1'000'000'000'000'000'000);
            carry = res / 1'000'000'000'000'000'000;
            i++;
        }
        while(i < x.data.size()){
    ...
    void output(){
        for(int i(data.size()-1); i >= 0; i--){
            if(i != data.size()-1) cout << setw(18) << setfill('0') << data[i];
            else cout << data[i];
        }
    }
};
